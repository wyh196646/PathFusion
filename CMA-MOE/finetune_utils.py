...
def process_survival_predicted_data(data, section='val'):
    """
    处理生存分析的预测结果数据，转成DataFrame格式以便保存为CSV。
    
    参数:
    - data: 字典，包括 'val_prob', 'val_label', 'val_slide_id', 等
    - section: 'val' 或 'test'

    返回:
    - result_df: 格式化后的DataFrame，index为slide_id，包含 prob、time、event 列
    """
    prob_list = data[f'{section}_prob']
    label_list = data[f'{section}_label']
    slide_id_list = data[f'{section}_slide_id']
    
    # 展平嵌套结构
    probs = np.array(sum(prob_list, []))  # shape: (N,)
    labels = np.array(sum(label_list, []))  # shape: (N, 2)
    slide_ids = sum(slide_id_list, [])  # shape: (N,)

    # 构建DataFrame
    result_df = pd.DataFrame({
        'slide_id': slide_ids,
        f'{section}_prob': probs.astype(np.float32),  # 更通用些
        f'{section}_time': labels[:, 0],
        f'{section}_event': labels[:, 1],
    })
